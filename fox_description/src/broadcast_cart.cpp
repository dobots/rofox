...
namespace transform_cart{
	...
	Transform::Transform(std::string name){ 
		name_ = name;
		// get model position
		cart_pos_cl_ = nh_.serviceClient<gazebo_msgs::GetModelState>("gazebo/get_model_state", "{model_name = fox_cart}"); 
		//Transform::callbc();
	};
	...
	void Transform::callbc(){	
		...
		if (cart_pos_cl_.call(modelstate_))	
            ...
            ROS_ERROR("Failed to call service get_model_state of fox_cart");
